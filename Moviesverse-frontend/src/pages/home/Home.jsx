...
import BuyMeACoffeeBanner from "./BuyMeACoffe/BuyMeACoffee";
...
const RecommendationSection = ({ item, title }) => {
  ...
  return <CustomLiked1 data={data} loading={loading} endpoint={item.media_type} title={title} />;
};
...
function Home() {
  ...
  const selectedWatchMovies = watchMovies
    ...
    .slice(0, 4 - selectedLikedMovies.length);
  const selectedWatchTv = watchMovies
    .filter((item) => item.media_type === "tv" && !selectedLikedIds.has(item.id))
    .slice(0, 4 - selectedLikedTv.length);
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Home page of moviesverse containing all the important endpoints, categories, and star components. Moviesverse - Explore and stream millions of movies, tv shows, animes, web shows etc for free."
        />
      ...
      <div className="homebanner" id="bgbirds">
        <div className="corousal">
          {showBMC && <BuyMeACoffeeBanner onClose={() => setShowBMC(false)} />}
          <Trending />
          <WatchHistory />
          {recommendationItems[0] && (
            <RecommendationSection
              key={recommendationItems[0].item.id}
              item={recommendationItems[0].item}
          ...
          {recommendationItems[1] && (
            <RecommendationSection
              key={recommendationItems[1].item.id}
              item={recommendationItems[1].item}
          ...
          {recommendationItems[2] && (
            <RecommendationSection
              key={recommendationItems[2].item.id}
              item={recommendationItems[2].item}
          ...
          {recommendationItems[3] && (
            <RecommendationSection
              key={recommendationItems[3].item.id}
              item={recommendationItems[3].item}
          ...
          {recommendationItems[5] && (
            <RecommendationSection
              key={recommendationItems[5].item.id}
              item={recommendationItems[5].item}
              title={recommendationItems[5].title}
            />
          )}
          <Collections />
          {recommendationItems[6] && (
            <RecommendationSection
              key={recommendationItems[6].item.id}
              item={recommendationItems[6].item}
              title={recommendationItems[6].title}
            />
          )}
          <Comedy />
          {recommendationItems[7] && (
            <RecommendationSection
              key={recommendationItems[7].item.id}
              item={recommendationItems[7].item}
              title={recommendationItems[7].title}
            />
          )}
          <VoteCount />
          {recommendationItems[4] && (
            <RecommendationSection
              key={recommendationItems[4].item.id}
              item={recommendationItems[4].item}
