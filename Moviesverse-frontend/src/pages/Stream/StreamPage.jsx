...
function StreamPage() {
  const fetchData = (url, setData, setLoading) => {
    const options = {
      ...
      headers: {
        ...
        Authorization:
          "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiZTJkMmM2YWZlNTMwYmJhYTEyZCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.zgqx7AWkKbNhLnQgNMY8u8Ei_9e34RRD-cAXyDMlfc8",
      },
    };
    setLoading(true);
    fetch(url, options)
  ...
  const { isOnStream, setSeason, setEpisode, setLanguage, setEndpoint, seLantEndpoint } = useContext(ValuesContext);
  const { user } = UserAuth();
  ...
  useEffect(() => {
    ...
    window.scrollTo(0, 0);
  }, [mediaType, id, season, episode, searchParams, setSeason, setEpisode, setLanguage, setEndpoint, seLantEndpoint]);
    ...
    const timerId = setTimeout(() => {
      const saveWatchHistory = () => {
        try {
          ...
          const cleanUserHistory = userHistory.filter(
            (item) => item && item.id && item.mediaType && item.timestamp && item.timestamp > sevenDaysAgo
          );

          const currentEpisodeDetails =
            mediaType === "tv" ? episodes.episodes.find((ep) => String(ep.episode_number) === String(episode)) : null;
  ...
  return (
    <>
      <Helmet>
        <title>{`Streaming ${detailsData?.name || detailsData?.title} ${
          mediaType === "tv" ? "Season " + season + " Episode " + episode : ""
        }, ${
          mediaType === "tv" ? mediaType.toUpperCase() + " Show" : mediaType.toUpperCase() || ""
        } | Moviesverse`}</title>
        <meta
          name="description"
          content="Stream page for movies and tv shows where users can watch their favourite content in different languages with multiple servers. Moviesverse - Explore and stream millions of movies, tv shows, animes, web shows etc for free."
        />
        <meta
          property="og:title"
          content={`Streaming ${detailsData?.name || detailsData?.title} ${mediaType || ""} | Moviesverse`}
        />
        <meta
